...
def SumPoloj(glav,n):
    ...
    summa = [0]*(kolvo_diag)
    for i in range(kolvo_diag):
        diag_changed = form_diag(glav[i],i,n)
        bool = False
        for j in range(len(diag_changed)):
            if diag_changed[j] >= 0:
                summa[i] += diag_changed[j]
                bool = True
        if not bool:
            summa[i] = -1
    return(summa)
...
произведение отрицательных элементов, так как их нет в матрице")
        ...
        poboch_ans = form_diag(poboch_diagonali[min_index],min_index,n)
...
положительных элементов, так как их нет в матрице")
        ...
        glav_ans = form_diag(glav_diagonali[max_index],max_index,n)
